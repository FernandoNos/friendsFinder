...
/**
    Publish an event with a picture using form-data library to facebook. Uses faked form submission.
    Ex.
        var event = {
...
**/
exports.postEvent = function(url, postData, callback){
    if (typeof url !== 'string') {
        var er = {message:'Graph api url must be a string'};
    ...
    if(postData.picture != undefined && postData.picture.length >0 ){
      var submitUrl = graphUrl+'/'+url;
      if(postData.edit && postData.postId){
        submitUrl = graphUrl+'/'+postData.postId;
      }
      //append form elements
      var form  = new forms();
      form.append('access_token', accessToken,{contentType:'text/plain'});
      form.append('name', _.stripTags(postData.name),{contentType:'text/plain'});
      form.append('start_time',postData.start_time,{contentType:'text/plain'});
      if(postData.end_time!=undefined){
        form.append("end_time",postData.end_time,{contentType:"text/plain"});
      }
      form.append('description',_.stripTags(postData.description),{contentType:'text/plain'});
      if(postData.ticket_uri != undefined && postData.ticket_uri.length >0){
        form.append('ticket_uri',postData.ticket_uri,{contentType:'text/plain'});
      }
      form.append('location',postData.location,{contentType:'text/plain'});
      form.append('picture', request(postData.picture));
      form.submit(submitUrl,function(err,response){
        if(err){
            callback(err,null);
        }else{
          var body = '';
...
/**
  Function to read user/page insights

  param:
  id: id of user/page/post to read insights on (user/page must have approved read_insights permission for the app)
**/
exports.readInsights = function(id, callback){
  if(id && typeof(id) ==='string'){
    var url = "/"+id+"/insights";
    return new Graph.get(url, {}, callback);
  }else{
    callback({success:false, message:"id must be a string"}, null);
  }
}

/**
  Makes Batch requests to facebook
  options={
    batch:[{"method":"GET", "relative_url":"{postId}/insights", post_id:"123456789"},{"method":"GET", "relative_url":"me/friends?limit=50"}]'

  }
**/
exports.batch = function(options, callback){
  if(options.batch != undefined && options.batch.length > 0 ){
    var url = graphUrl+"?access_token="+accessToken+"&batch="+qs.stringify(options.batch);
    request(url, function (err, res, body) {
      if (err) {
        ...
        if(!lo.isObject(body) && typeof(body) === 'string'){
          body = JSON.parse(body)  
        }
        lo.each(options.batch, function(item, index, list){
          if(item.post_id){
            if(lo.isArray(body)){
              body[index].post_id = item.post_id
            }
          }
        });
        callback(null, body);
    ...
    callback({success:false, message:"No items sent in batch."});
  }
}
/**
